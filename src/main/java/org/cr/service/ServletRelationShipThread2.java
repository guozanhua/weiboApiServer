 ...
 * 
 */
public class ServletRelationShipThread2 implements Runnable {

	static Logger log = Logger.getLogger(ServletRelationShipThread2.class.getName());

	private String access_token;
	private UserDaoImpl userDaoImpl = new UserDaoImpl();
	private RelationPathDaoImpl relationPathDaoImpl = new RelationPathDaoImpl();

	public ServletRelationShipThread2(String access_token) {
...
//	UserRelationNode userTree;
	...
	List<RelationPathBean2> relationlists = new ArrayList<RelationPathBean2>();
...
//		userTree.setUserChildren(new ArrayList<UserRelationNode>());
		...
		try {																	//这里需要改成200 不然此人社交网络不完整
			userWapper = friendships.getFriendsBilateral(printedUid, 0,new Paging(1, 50));
...
//					if()
				...
				if (this.getRelationListsChildsFather(u) != null) {
					RelationPathBean2 relationPathBean2 = this.getRelationListsChildsFather(u);
					...
					// 插入没有孩子的path
					relationlists.add(new RelationPathBean2(printedUid, printedUid,
									u.getUid(), xlv1s + "", ylv1s + "", xlv1e+ "", ylv1e + "", u.getName(),5 + "", 1 + ""));
